...
4.2.1
    · 修复「重启 Harness 后界面一直显示正在重启、像是起不来」：查出两个独立原因
      —— ① 重启走的是旧路径，绕过了 4.2 新增的启动前体检与失败自愈，
         插件/配置有问题时重启必然卡到 90 秒超时；现在重启与首次启动走同一条自愈路径
      —— ② 「正在重启 Harness……」这条横幅弹出后没有任何代码把它收掉，
         于是一直挂在页面上；现在会等服务确实回来（或明确失败）再换掉，
         并给出「已重启完成」或具体失败原因
    · 启动器令牌文件自愈：logs\\launcher.token 被删掉后，还活着的实例会自己补写回来，
      不再出现重启后一直 401 的情况
4.2.0
    · 新增「启动自检」：启动 Harness 之前先自动检查本体与插件依赖，把「起不来」的原因提前找出来
      —— Harness 本体是否完整、profile 配置能否解析、插件是否都装齐、插件自己的依赖是否缺、
        配置补丁（cordis.patch.yml）有没有语法错误、会话日志有没有损坏
    · 自动修复：缺失的依赖自动补装；配置补丁写坏（如重复键）自动备份并修好；实在装不上的插件自动禁用
      所有改动都先备份到 data\\_preflight-backup\\，可回滚
    · 启动失败自动自愈：Harness 没起来时自动从日志里定位「是哪个插件害的」，禁用该插件后重试（最多 3 轮）；
      定位不到具体插件时不会瞎猜、不会乱删，而是如实报错
    · 启动失败报错更快更准：进程秒退时立刻报「进程已退出」并附上真实原因，不再傻等 90 秒超时
    · 维护页新增「启动自检」区块：展示检查结论、本次自动处理了什么，并提供「重新检查」与「一键修复」按钮
4.1.9
    · 去掉免登录开机：此前「计划任务（开机触发 + SYSTEM）+ 登录项」双机制会让两个实例并存（SYSTEM 会话 0 + 登录会话 1），
      带来三个问题——开机时 SYSTEM 实例忙于加载本地模型、登录实例附着探测超时导致界面提示「端口已占用」；
      服务归属混乱（本地模型归 SYSTEM 实例管、登录实例拿不到它的 pid）导致点「停止全部」停不掉；
      SYSTEM 实例在会话 0 没有界面、用户看不到它在管什么
    · 现在只保留登录项：登录后拉起启动器（托盘/窗口），配合「开机自动启动服务」自动拉起本地模型与 Harness；
      开机到登录之间不跑任何东西，「免登录也要能用」改由用户自己的远程控制方案解决
    · 每次开启/读取自启设置时都会顺手删除旧版本遗留的「DSH Launcher Autostart」计划任务
4.1.8
    · 修复托盘「退出」点了没反应：托盘菜单曾先把退出标志置位再调用退出流程，而退出流程开头会因该标志直接返回，
      导致什么也不做（死锁）；现统一由退出流程自己置位，并加 8 秒兜底强制退出，避免子进程卡住时退不掉
    · 附着模式下退出会明确提示「后台服务由开机实例托管、仍在运行」，不再让人误以为没退干净
    · 修复免登录开机的双实例冲突：附着探测失败时不再退化成「换随机端口 + 已有实例退出」，
      而是重试探测；确认端口被同目录的自家实例占用时提示并退出，避免留下占着端口却不响应的僵尸实例
    · 开机自启带服务加跨实例文件锁：SYSTEM 开机实例与登录实例不再各拉一份本地大模型（此前会导致
      同一模型被加载两次、显存争抢，表现为「切换本地模型卡住」）
    · 保留「一台机器跑两套独立 harness」场景：端口被其它安装占用时仍按原逻辑改用随机端口
4.1.7
    · 开机自启免登录：「开机自动启动」改为计划任务（开机即触发、以 SYSTEM 运行，无需登录）+ 登录项双机制；
      登录后用户会话里的实例检测到开机实例已持有后端时会附着其界面（托盘/窗口可用，不占端口、不重复拉服务）
4.1.6
