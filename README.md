# DSH 启动器

本地图形化启动器：一键管理 DeepSeek Harness（dsh）+ 本地大模型（llama.cpp）+ 下载任务，Electron 桌面窗口（可回退纯 Node 浏览器模式）。

## 更新日志

### 4.2.1
- **修复「重启 Harness 后界面一直显示正在重启、像是起不来」**（2026-09-24 用户反馈）。查出两个独立原因，都已修：
  - **重启绕过了启动自检**：从对话里点「重启 Harness」走的是旧路径，**没有**走 4.2 新增的启动前体检与失败自愈——所以插件或配置有问题时，重启必然卡死到 90 秒超时。现在重启与首次启动走同一条「先体检、失败就归因重试」的路径。
  - **横幅永远不会消失**：界面弹出「正在重启 Harness……」之后，**没有任何代码把它收掉**，于是横幅一直挂在页面上，看起来就像卡在重启中。现在会等到服务确实回来（或明确失败）再把横幅换掉，并给出「已重启完成」或具体失败原因。
- **顺手修好重启后可能的 401**：令牌文件 `logs\launcher.token` 被删掉时，还活着的启动器实例现在会自己补写回来，不用等到下次重启启动器。
- 自测从 74 项增加到 **83 项**，新增用例专门盯住「重启必须走体检与自愈」「重启横幅必须能收起」这两条，防止再退化。

### 4.2.0
- **新增「启动自检」——启动 Harness 前先自动体检**：把「dsh 起不来」的原因在启动前就找出来，不再靠事后翻日志。检查六项：① Harness 本体是否完整；② profile 配置（`package.json`）能否解析；③ 声明的插件是否都装齐、能否被解析到；④ 插件自己的**运行时依赖**是否缺；⑤ 配置补丁 `cordis.patch.yml` 有没有语法错误；⑥ 会话日志有没有损坏（会让 dsh 报 `corrupt session log`）。
- **自动修复**：缺失依赖自动补装（`dsh plugin --profile web install`）；配置补丁写坏（如重复键导致 `duplicated mapping key`）自动**备份并修好**；实在装不上的插件**自动禁用**并在维护页列出。所有改动都先备份到 `data\_preflight-backup\`，可回滚。
- **启动失败自动自愈**：Harness 没起来时，自动从本次启动的日志里**定位是哪个插件害的**（`failed to import loader entry … (<包名>)`，或从报错路径反查插件目录），禁用该插件后**自动重试**（最多 3 轮）。定位不到具体插件时**不瞎猜、不乱删**，如实报错；Harness 自身内部包缺失也不会被误判成插件问题。
- **启动失败报错更快更准**：dsh 因插件/配置问题起不来时是**秒级退出**，以前会傻等到 90 秒超时才报错（感觉像卡住）。现在一旦发现进程已退出就立刻报「Harness 进程已退出」并附上真实日志原因。实测同一故障从 90 秒缩短到约 7 秒。
- **维护页新增「启动自检」区块**：展示最近一次启动的体检结论、本次自动处理了什么，并提供「重新检查」（只读，不改任何文件）与「一键修复」按钮。新增接口 `GET /api/preflight`、`POST /api/preflight/repair`、`POST /api/preflight/enable`（恢复被禁用的插件）。
- 自测从 58 项增加到 **74 项**，新增用例覆盖失败归因、YAML 检查（含「同级列表项不算重复键」这类曾误报的坑）、插件禁用与恢复回滚。

### 4.1.9
- **去掉免登录开机（回到只登录后启动）**：此前用「计划任务（开机触发 + SYSTEM）+ 登录项」双机制实现无人登录也能跑服务，但它让**两个实例并存**（SYSTEM 会话 0 + 登录会话 1），带来三个问题：① 开机时 SYSTEM 实例正忙于加载本地大模型，登录实例的附着探测容易超时 → 退化成随机端口 → 界面提示「端口已占用」；② 服务归属混乱——llama 是 SYSTEM 实例的子进程，登录实例的 `llm.pid` 却是空的，界面上点「停止全部」停不掉；③ SYSTEM 实例在会话 0 没有界面，用户看不到它在管什么。现在**只保留登录项**：登录后拉起启动器（托盘/窗口），配合「开机自动启动服务」自动拉起本地模型与 Harness。开机到登录之间不跑任何东西；「免登录也要能用」改由用户自己的远程控制方案解决。
- **自动清理历史遗留的计划任务**：`setAutoStart` / `isAutoStart` 每次都会顺手删除旧版本创建的 `DSH Launcher Autostart` 任务，避免升级后它继续把 SYSTEM 实例拉起来。

### 4.1.8
- **修复托盘「退出」点了没反应**：托盘菜单曾先置位退出标志、再调用退出流程，而退出流程开头会因该标志直接返回 → 什么也不做（死锁）。现统一由退出流程自己置位；并加 **8 秒兜底强制退出**，避免子进程卡住时退不掉。
- **附着模式退出说明**：登录后附着的窗口退出时，会明确提示「后台服务由开机实例托管、仍在运行」，不再让人误以为没退干净。
- **修复免登录开机的双实例冲突**：附着探测改为**重试**（开机时 SYSTEM 实例的令牌文件可能尚未落地，一次探测容易误判为「没有已运行后端」）；若确认端口被**同目录的自家实例**占用，则提示并退出，不再退化成「换随机端口 + 已有实例退出」——那正是「占着 7610 却不响应」的僵尸实例成因。
- **开机自启带服务加跨实例文件锁**：SYSTEM 开机实例与登录实例不再各拉一份本地大模型（此前会导致同一模型被加载两次、显存/内存争抢，表现为**切换本地模型卡住**）。锁带过期时间，进程崩溃留下的陈锁会被后续实例接管。
- **保留「一台机器跑两套独立 harness」场景**：端口被**其它安装**占用时，仍按原逻辑改用随机端口，行为不变。

### 4.1.7
- **开机自启免登录**：「开机自动启动」不再只写登录项（`HKCU\...\Run`，必须登录后才跑），而是同时创建一个**计划任务**（开机即触发、以 SYSTEM 运行）——机器一开机启动器就在后台静默运行，**无需任何人登录**（配合「开机自动启动服务」即可开机即起本地模型 + Harness）。用户登录后，登录项仍会在用户会话里拉起一个实例；它检测到开机实例已持有后端时会**附着**其界面：托盘图标和窗口照常可用，但不占用端口、不重复拉起服务，退出也只关自己的窗口、不影响开机实例里的服务。

### 4.1.6
- **启动器去掉「对话管理」，删除功能搬到 Harness 网页端**：主页不再显示对话管理卡片（列表/删除/清空），相关后端接口一并移除。改为在 dsh 网页（3080）内新增对话管理：右侧边栏新增「对话」标签（列出全部对话——标题/时间/大小，可逐条删除、一键清空），且每个对话的标题栏新增删除按钮。删除直接清理 dsh 会话存储（`data\sessions` 下对应目录），**正在运行中的对话会拒绝删除**；删除后左侧对话树自动刷新，若删的是当前对话会自动跳到最近的其他对话。

### 4.1.5
- **重启后任务自动继续（重启续跑）**：重启 Harness 不再把进行中的对话「停在那」。新实例恢复 dsh 就绪后，启动器会自动通过 dsh 网页协议向目标会话发一条「继续」用户消息，让 agent 接着跑，无需人工点一下。目标会话：重启请求可显式指定（`POST /api/launcher/restart` body `{sessionId, resumeText}`），缺省自动取**最近更新过的对话**（旧实例是旧版本、没写意图文件时也照常生效）。会话存于磁盘、重启不丢。

### 4.1.4
- **「开机自动启动服务」独立选项**：设置页「启动器」区新增独立开关，与「开机自动启动」分开控制。勾选后，开机（静默模式）时除进托盘外还会自动启动本地大模型和 Harness；不勾选则开机只进托盘、服务保持停止。同样只在干净开机时生效，「重启生效」仍按退出前状态恢复。

### 4.1.3
- **开机自启带起服务**：开启「开机自动启动」后，开机时启动器（静默模式）除了进托盘，还会**自动启动本地大模型和 Harness**，开机即用。只在干净开机（无服务状态文件）时生效；「重启生效」仍优先按「恢复退出前运行中的服务」执行，你手动停掉的服务不会被误拉起。

### 4.1.2
- **修复「打开目录」按钮点了没反应**（关于页的数据/日志/模型/配置/启动器目录）：打开目录的助手脚本原来用「分离式（detached）PowerShell」方式启动，而 Win11 24H2 上这种启动法会让 powershell 静默退出（退出码 0、脚本根本不执行），导致按钮毫无反应。已去掉 `detached` 启动参数（助手脚本正常打开目录并把窗口提前），并加了启动失败日志便于排查。

### 4.1.1
- **修复维护页「环境诊断 / 版本管理」空白**：主页「本地模型快捷卡片」引用了只定义在维护页里的变量 `activeModel`，严格模式下状态刷新一执行就抛 ReferenceError，还被整体误报成「后端不可用」——诊断列表卡在「加载中……」、版本表空着。现在 `activeModel` 从状态数据里取、主页/维护页共用；渲染出错时显示真实错误信息，不再误报后端不可用。
- **修复版本管理「最新 未检查 · 已是最新」矛盾**：组件未检查过更新时，`updateAvailable` 现在返回 `null`（未知）而不是 `false`，不再提前显示「已是最新」、也不再提前禁用「更新」按钮。
- **内置更新日志补上缺失的 4.1.0 条目**（维护页「更新日志」展开可见）。

### 4.1.0
- **修复「dsh 起不来」**：web profile 的 `cordis.patch.yml` 里 webserver 段会覆盖 `dsh web --port`，导致改 `DSH_PORT` 不生效、端口回退也撞回原端口（EADDRINUSE）；现在启动前把该段 port 同步为实际端口。
- **修复启动失败报错**：dsh「先监听后崩溃」时 state.dsh 已被清空，原先给 state.dsh.url 赋值会抛 TypeError 掩盖真实原因；现在启动失败能看到实际报错。

### 4.0.0
- **全新图标**：exe / 窗口 / 任务栏 / 网页标签统一为「深蓝底白色闪电」（`gui\icon.png` + `launcher.ico`）。
- **系统托盘**：关窗收进托盘、服务不停（本地大模型后台继续运行）；托盘菜单可「打开启动器 / 退出」，只有明确退出才停全部服务。
- **开机自启**：设置页新增「开机自动启动」开关，开机后台静默运行（`--silent`，只进托盘不弹窗）；写注册表 `HKCU\...\Run`。
- **更新提示**：启动时自动检查（GitHub 最新 release / 清单），发现新版本主页顶部横幅提示，可一键更新或忽略本次。
- **版本切换**：维护页可在「已安装版本 + GitHub 全部历史版本」之间切换（任意历史版本自动从 GitHub 下载）；harness / llama / node 同样支持按已安装备份切换 + 「更新说明」外链。
- **对话管理**：主页新增「对话管理」卡片 —— 列出全部对话（id / 时间 / 大小），逐条删除 / 一键清空 / 打开对话文件夹。
- **模型下载**：下载条模型名移到进度条上方；新增「删除任务」按钮（停下载 + 删临时文件 + 移除记录）。
- **主页快捷**：组件版本卡片加「启动器地址 + 复制」；底部加「对话管理」「本地模型」快捷卡片。
- **关于页**：新增启动器主要功能说明 + GitHub 地址；目录按钮改用 `explorer.exe` 直开（修复「打开目录」按钮偶发不激活）。

### 3.6.0
- **本地模型支持视觉（图像理解）**：设置页新增「视觉投影器 LLM_MMPROJ」（相对或绝对路径，留空 = 纯文本）。配置后「启动/重启本地模型」自动追加 `--mmproj` 参数加载视觉投影器权重（llama.cpp b10941+ 自带 mtmd 多模态后端），同时 `settings.yaml` 的本地模型条目声明 `inputModalities: [text, image]`，Harness 即可向本地模型发送图片（如 `read_image` 工具读取本地图片后提问）。投影器文件在模型广场下载（mmproj 也是 `.gguf` 后缀，可正常搜索/下载）。
- `/api/status` 的本地模型状态新增 `mmproj` 字段（已配置的投影器路径，未配置为 null）。

### 3.5.4
- **最大输出改独立设置**：设置页新增「最大输出 LLM_MAXTOKENS」下拉菜单（自动 / 4K / 8K / 16K / 32K / 64K / 128K）。自动时按当前模型自身上限算（不再写死 32K，换模型自动适配）。
- **自动时提示实际值**：上下文长度 / 最大输出选「自动」时，下方实时显示实际生效值（如 实际 192K / 实际 32K），方便确认。

### 3.5.3
- **设置页「上下文长度」改下拉菜单**：可选 自动（按显存）/ 32K / 64K / 128K / 192K / 256K，默认值改为 auto，不用再手填数字。

### 3.5.2
- **上下文长度 / 最大输出自动调节**：LLM_CTX=auto 时按显存 + 模型结构（gguf 探测 KV 缓存）自动选出能容纳的最大上下文（本机 32GB：128K→192K），最大输出自动对齐模型上限 32K。

### 3.5.1
- **模型最大输出 maxTokens 8K→16K**：长回答（写文章 / 长代码）不再被截断；短问答不受影响（16K 是上限，答完即停）。

### 3.5.0
- **下载提速**：中国镜像（默认 hf-mirror，可用 DIRECT_HOSTS 追加）自动直连、绕开海外代理。实测模型下载 4.5→21 MB/s（快 4.7 倍）；GitHub 等其余源仍走 DOWNLOAD_PROXY（小文件快 6 倍）。
- **设置新增「直连主机 DIRECT_HOSTS」**（逗号分隔），默认自动包含 HUB_MIRROR 主机。

### 3.4.0
- **本地模型状态修正（外部启动）**：本地大模型由启动器之外的路径启动（如 `dsh-tasks` 插件的 Auto 本地切换直接拉起 `llama-server`）时，pid 文件为空，状态页此前误报「未启动」而实际可用。现 `/api/status` 在 pid 缺失时补一次健康探测（`/v1/models` 200 即视为运行中，并回填端口占用者 pid）。
- **「启动本地模型」报错更准**：端口被「同一个」本地模型占用（健康检查通过）时，提示改用「重启本地模型」，不再误导用户去改 `LLM_PORT`。

### 3.3.2
- **下载代理**：设置页新增 `DOWNLOAD_PROXY`（如 `http://127.0.0.1:10808`，留空为直连）。直连 GitHub 不稳定时，所有 curl 下载（启动器更新包 / 模型文件 / node、llama.cpp 运行时）与更新清单请求都改经代理；代理模式下 HEAD 与清单抓取走 curl（Node fetch 不吃代理）。

### 3.3.1
- **exe 提示文案修正**：缺失必要组件时不再提示「请先运行 setup.bat」（该脚本已随 V2 退役），改为「请检查 DSH 启动器是否完整解压」。

### 3.3.0
- **目录结构去 -v3**：`server/ gui/ electron/` + 入口 `launcher.bat`、`启动器.bat/vbs`、`dsh-launcher.exe`、`launcher.cs`、`test/`；electron 运行时移至 `runtime\electron`（与 `runtime\node` 并列）；V2 组件退役（源码备份至 GitHub 仓库 `stoneyf/dsh-launcher-v2`）。
- **GitHub 更新源**：自更新默认走 GitHub Releases —— 维护页「检查更新」读取 `https://raw.githubusercontent.com/stoneyf/dsh-launcher/main/launcher-manifest.json`（稳定地址，永远指向最新 release 的 zip）。发版用 `scripts\publish.ps1`（打包 + manifest + commit + tag + release 一键完成）。网络不佳时可在设置里把更新源临时改为 ghproxy 前缀或本地文件。
- **令牌文件**：`logs\launcher.token`（每次启动重新生成）。

### 3.2.0
- **启动器重启续会话**：`POST /api/launcher/restart` → 退出并在 quit 事件里 relaunch；shutdown 时把 dsh/llm 运行态写入 `data/launcher-services.json`，新实例启动后自动拉起对应服务，浏览器 cookie 保证会话续接。
- **关闭窗口再打开也能自动恢复（修复）**：此前状态文件只在「主动重启」路径写入，普通关窗/重开（`window-closed`）不会恢复 dsh。现改为：任何退出原因下只要有服务在运行就写状态文件；恢复前 `waitPortFree` 等端口释放，避免「端口已被占用」冲突。Electron 后端的 console 输出同步转发到 `logs\electron.log`（GUI 模式下不再静默失败）。
- **自测扩充**：selftest 新增第 ⑨ 阶段「第二实例自动恢复」，共 32 项检查全部通过；另附 `test\copy-restart-test.mjs` 真实环境副本测试（独立端口 7611/3081：起 dsh → 重启 → 新实例自动恢复 dsh → 状态文件消费，8 项通过）。
- **模型广场介绍 + 硬件建议**：搜索结果带派生简介（参数规模 / MoE 激活 / 能力标签，HF 搜索接口本不带正文）；选中仓库后展示 README 简介 + license，并按本机内存 / NVIDIA 显存对每个 GGUF 量化文件评级（可上显存 / GPU+CPU / 内存宽裕 / 紧张 / 超出），给出推荐下载文件与一句话建议。
- **关于页目录按钮**：改用 PowerShell helper（`open-dir-helper.ps1`）枚举 `CabinetWClass` 窗口，命中已存在目录则前置显示、并关闭同目录的重复/隐藏窗口；Win11 per-window-process Explorer 下不再「按钮失效」。
- **启动器 exe**：新增 `dsh-launcher.exe`（C# 无控制台入口，`launcher.cs` 用 `csc` 编译），双击即用；无 Electron 时回退纯 Node 浏览器模式。

### 3.1.0
- **启动器自更新**：维护页「启动器版本」卡片支持检查更新 / 更新 / 回滚 / 重启生效；清单来自设置里的「更新源」（http(s) 地址或本地文件路径），更新前自动备份到 `launcher-backup-{版本}`（保留 3 份），回滚一键恢复。
- **主题**：深色 / 浅色 / 跟随系统（设置页即时生效，持久化到 `config`），控制台与日志框配色随主题切换。
- **模型广场增强**：进页面自动搜索、搜索结果分页「加载更多」、搜索历史 chips（≤8 条）、文件列表可按文件名筛选 + 按大小/名称排序、量化标签与「已安装」标记、仓库页/文件直链外链、下载条实时显示速度与剩余时间。
- **隐藏 cmd 窗口**：双击入口走 `启动器.vbs` / `dsh-launcher.exe`（无 cmd 窗口）；`launcher.bat` 为 ASCII 引擎，`启动器.bat` 为窗口版排障入口。
- **关于页修复**：四个目录按钮改用 `cmd /c start` 打开（解决 explorer 复用已有实例导致窗口不激活的「失效」问题）；新增「打开配置目录」、版本号与 Changelog 展示。

### 3.0.0
- 首个独立版本：启动器编排重启（202 + SSE 阶段推送）、陈旧状态自愈、孤儿端口查杀、GUI 实时重启进度条、版本管理（harness/llama/node 检查/更新/回滚）、模型广场、下载续传。

## 核心设计：agent 重启 dsh 不断会话

**核心问题：agent（harness 里的会话）重启 dsh 时，自己的命令进程会随 dsh 一起死，重启永远完不成。** 方案：重启由启动器编排，agent 只发一个 API 请求。

| 能力 | 说明 |
|---|---|
| `POST /api/services/dsh/restart` | 立即返回 **202**，后台完成：杀 dsh → 查杀 3080 上的孤儿占用者 → 等端口释放（20s）→ 由启动器 spawn 新 dsh → 解析新 token URL |
| `POST /api/services/llm/restart` | 同上（llm 用 30s 就绪等待） |
| `GET /api/services/events` | SSE 实时推送重启阶段：`stopping → killing-port-holder → waiting-port → starting → loading → ready`（或 `error` + 原因） |
| `POST /api/launcher/restart` | 启动器自身重启（自更新/回滚后生效）：写服务状态 → 退出 → relaunch → 新实例自动恢复 dsh/llm |
| 陈旧状态自愈 | start 前先检查内存状态与 pid 文件/进程存活，不一致自动清除（`stale-state-cleared`） |
| 端口查杀 | 重启前用 `netstat -ano` 找出占用目标端口的孤儿进程并整树杀掉（排除 dsh 自身 pid） |
| 严格端口 | 重启不走端口回退（不再悄悄落到 3081），端口占着就报错 |
| 并发保护 | 同一服务重启中重复提交 → **409** |

浏览器会话不断的原因：dsh 用**签名 cookie**（30 天有效，密钥在 `data/.credentials.yaml`），dsh 重启后密钥还在 → 页面短暂断连后自动重连，用户无感。

### agent 重启协议（给 harness 里的会话看）

agent 的 shell 环境变量里有 `DSH_HOME`（指向启动器的 `data\`），启动器根目录 = `DSH_HOME` 的父目录：

```powershell
$root = (Split-Path $env:DSH_HOME -Parent)
$token = (Get-Content (Join-Path $root 'logs\launcher.token')).Trim()
$port = 7610   # config\launcher.env 的 LAUNCHER_PORT
Invoke-RestMethod -Method Post -Uri "http://127.0.0.1:$port/api/services/dsh/restart" -Headers @{Authorization="Bearer $token"}
# 可选：轮询 GET /api/status 直到 restarting.dsh 为 ready 或消失
```

要点：**不要自己 taskkill dsh，不要自己 spawn dsh** —— 一切交给启动器。

## 目录结构

```
D:\dsh-launcher\
  server\        后端（Node 原生 http，零依赖；含 open-dir-helper.ps1）
  gui\           前端 SPA
  electron\      Electron 壳（主进程即后端宿主）
  runtime\       运行时：node\ 与 electron\（gitignore，setup 安装）
  test\          selftest.mjs + mock-dsh.js + copy-restart-test.mjs
  scripts\       publish.ps1（GitHub 发版）
  dsh-launcher.exe  双击启动入口（C# 无控制台，推荐）
  启动器.vbs      双击启动（隐藏窗口，exe 的替代品）
  启动器.bat      窗口版排障入口（调用 launcher.bat）
  launcher.bat    ASCII 启动引擎：有 Electron 用 Electron，否则纯 Node + 浏览器
  launcher.cs     启动器 exe 源码（csc 编译，见下）
  launcher-manifest.json  GitHub 更新清单（publish.ps1 生成并提交）
  config\ data\ logs\ models\ harness\ llm\ plugins\  运行时数据（gitignore）
  launcher-backup-{版本}\  自更新/回滚备份（保留 3 份）
```

编译启动器 exe（无需 VS）：

```powershell
C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe /nologo /target:winexe /optimize /codepage:65001 /out:dsh-launcher.exe launcher.cs
```

## GitHub 更新

- 仓库：`stoneyf/dsh-launcher`（私有）；V2 源码备份：`stoneyf/dsh-launcher-v2`（私有）。
- 发版：`scripts\publish.ps1` → 打包受管目录（`server/ gui/ electron/` + 入口文件）为 `launcher-v{版本}.zip` → 生成 `launcher-manifest.json`（version/notes/url 指向 release 资产）→ 提交清单到 main → 打 tag `v{版本}` → 创建 GitHub Release（附件 zip + manifest）。
- 本机更新源（`config\launcher.env`）：
  ```
  LAUNCHER_UPDATE_URL=https://raw.githubusercontent.com/stoneyf/dsh-launcher/main/launcher-manifest.json
  ```
  main 分支的 manifest 永远描述最新 release，维护页「检查更新」即可发现新版本；安装后点「重启」生效（更新前自动备份，可回滚）。
- 网络不佳：把 `LAUNCHER_UPDATE_URL` 临时改为 `https://ghproxy.com/https://github.com/...` 前缀或本地 zip/manifest 文件路径。

## 自测

```powershell
D:\dsh-launcher\runtime\node\node.exe test\selftest.mjs
```

用 fixture 隔离（mock dsh 监听 3999，后端 7999），覆盖 9 个阶段 32 项检查：fixture → 后端启动 → 状态/token → dsh 启动 → dsh 重启全流程（202/409/SSE 阶段/新 pid/新 URL）→ 陈旧状态自愈 → 孤儿端口查杀 → llm 错误路径 → 第二实例自动恢复。`test\selftest.steps.log` 有无缓冲步骤日志。

## 已知限制

- `portHolderPids` 用 `netstat` 解析（无额外依赖，够用；极端并发下可能漏判，20s 等待兜底）。
- llm 重启的错误详情保留 120s 供 GUI 展示；dsh 的 ready 状态保留 15s。
- 同一台机器不能同时跑两个启动器实例监听同一 LAUNCHER_PORT（EADDRINUSE 会随机端口回退并写日志）。
