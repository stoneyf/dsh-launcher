...
### 4.1.5
- **重启后任务自动继续（重启续跑）**：重启 Harness 不再把进行中的对话「停在那」。重启请求（`POST /api/launcher/restart`）现在可带目标会话（缺省取最近更新过的对话），新实例恢复 dsh 就绪后，启动器会自动通过 dsh 网页协议向该会话发一条「继续」用户消息，让 agent 接着跑，无需人工点一下。会话本身存于磁盘、重启不丢。
